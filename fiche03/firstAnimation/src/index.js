...
let animationRun = requestAnimationFrame(drawOneFrame);

function drawOneFrame() {
  main.addEventListener("click",pauseAnimation);
  ...
  let x = requestAnimationFrame(drawOneFrame);
  let main = document.querySelector('#page');

  // main.addEventListener("click", requestAnimationFrame((() => cancelAnimationFrame(drawOneFrame()))));

  // Slow the animation down via setTimeout
  // requestAnimationFrame(() => setTimeout(drawOneFrame,1));
}
